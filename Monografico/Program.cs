...
namespace Monografico
{
    public class Program
    {
        ...
        {
            var usuario = await _userManager.FindByNameAsync("Albert");
            if (usuario == null)
            {
                ...
                Usuario user = new Usuario
                {
                    UserName = "Albert",
                    Nombre = "Albert Lopez",
                    Email = "Albert@hotmail.com",  //El userManager no permite correos repetidos.
                    PhoneNumber = "8093432342",
                    LockoutEnabled = false

                };
                var result = await _userManager.CreateAsync(user, "lopez12347.");
